...
int store[100];
char looping;
bool checkFinal(char initial,char *final,int len){
    bool res = false;
    for(int i=0; i<len; i++){
        if(initial==final[i])return true;
    }
    return false;
}
void stateUpgrade(int num_states,int num_symbol,char *setofstate,char *alphabet,char initialstate,char *finalstate,char transition[num_states][num_symbol][10],int num_transition[num_states][num_symbol],int num_final,int index){
    if(index==len && checkFinal(initialstate,finalstate,num_final)){
        result = true;
    ...
    if(i==num_symbol)i=num_symbol-1;
    if(num_transition[initialstate-'a'][i]==0)return;
    int k=0;
    store[p++] = initialstate;
    while(k<num_transition[initialstate-'a'][i] && !result){
        stateUpgrade(num_states,num_symbol,setofstate,alphabet,transition[initialstate-'a'][i][k],finalstate,transition,num_transition,num_final,index+1);
        k++;
    }
    return;
}
int main(){
    ...
    stateUpgrade(num_states,num_symbol,set_of_state,alphabet,initial_state,final_states,transition_table,num_transition,num_final,0);
